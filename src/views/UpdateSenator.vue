					...
					<v-card>
						<v-card-text class="py-10 px-8">
							<ValidationObserver ref="form" v-slot="{ handleSubmit, reset }">
								<form
									@submit.prevent="handleSubmit(updateSenator)"
									@reset.prevent="reset"
								>
									<ValidationProvider
										v-slot="{ errors }"
...
export default {
	...
	methods: {
		async getSenatorById() {
			this.errored = false
			this.loading.input = true

			const senator = await SenatorService.findById(this.$route.params.id)
				.catch((err) => {
					console.log(err)
					this.errored = true
				})
			...
			if (typeof senator === 'undefined') return

			this.data = senator.data.data[0]
		},
		async getStates() {
			this.errored = false
			this.loading.input = true
		...
		async updateSenator() {
			...
			const senator = await SenatorService.updateById(this.data)
				.catch((err) => {
			...
			this.$store.dispatch('message', 'Senator successfully updated')
		},
	},
	mounted() {
		this.getStates()
